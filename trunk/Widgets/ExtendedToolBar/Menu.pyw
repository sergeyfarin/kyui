...
class KyMenu(QMenu):
    ...
    def addNamedSeparator(self, title : str, icon : QIcon = None):
    ...
    def insertNamedSeparator(self, before : QAction, title : str, icon : QIcon = None):
        act = self.insertSeparator(before)
        act.setText(title)
        if icon:
            act.setIcon(icon)
        act.setData('named')
    ...
    def sizeHint(self) -> QSize:
        ...
        for act in self.actions():
            if act.isSeparator() and act.data() == 'named':
                ...
                #2 is the default height for separators, so we have to subtract that
                if size.height() > 2: sz.setHeight(sz.height() + size.height() - 2)
        return sz
