...
class KyStyle(QStyle):
    ...
    def __init__(self, styleName = None):
        ...
        if not styleName:
            ...
            self.__customStyle = KyStyleFactory.create('Plastique')
        elif styleName not in KyStyleFactory.keys() or styleName not in QStyleFactory.keys():
            qWarning('Style {} is not a valid style. Using Plastique style.', styleName)
            self.__proxy = QStyleFactory.create('Plastique')
            self.__customStyle = KyStyleFactory.create('Plastique')
        else:
            self.__proxy = QStyleFactory.create(styleName)
            self.__customStyle = KyStyleFactory.create(styleName)
